        ...
        return (
            <div className="bg-white rounded-xl shadow-sm border-l-4 border-emerald-500 p-4">
                <p className="text-sm font-semibold text-slate-900">
                    No Study Plan Needed!
                </p>
                <p className="text-xs text-slate-500 mt-1">
                    All your recent subject scores are above 85%. Keep up the
                    great work! 🎉
                </p>
            </div>
        );
                        ...
                        <summary className="flex items-center justify-between px-3 py-2.5 cursor-pointer bg-slate-50 hover:bg-slate-100">
                            <div className="flex items-center gap-3">
                                ...
                                <span className="inline-flex items-center rounded-full bg-amber-100 text-amber-700 text-[11px] px-2 py-0.5 font-medium">
                                    Your Score:{" "}
                                    {subjectPlan.percentage.toFixed(1)}%
                                </span>
                            </div>
                        ...
                        <div className="px-3 py-2 space-y-2">
                            {subjectPlan.chapters.length > 0 ? (
                                ...
                                <p className="text-xs text-slate-500 pb-1">
                                    Syllabus topics for this subject could not
                                    be loaded. Please ensure the syllabus has
                                    been uploaded by your teacher.
                                </p>
                            )}
                        </div>
                    </details>
...
const Dashboard = () => {
    ...
    const loadMentorStatus = async () => {
        try {
            ...
            const res = await axios.get(
                `${apiUrl}/api/student/mentor-status/${user.id}`
            );
    ...
    const generateStudyPlan = async () => {
        ...
        try {
            ...
            const planPromises = lowScoringSubjects.map(async (mark) => {
                try {
                    const topicsRes = await axios.get(
                        `${nlpApiUrl}/syllabus/topics/${encodeURIComponent(
                            mark.subject
                        )}`
                    );
                    ...
                    console.error(
                        `Could not fetch topics for ${mark.subject}:`,
                        topicError
                    );
            ...
            setMarksError(
                e.response?.data?.message || "Failed to load marks."
            );
        } finally {
            setPlanLoading(false);
        }
    ...
    const trend = useMemo(() => {
        ...
        const curr = Number(
            latestEntry?.predicted_risk_percentage || 0
        );
        const prev = Number(
            prevEntry?.predicted_risk_percentage || 0
        );
            ...
            <div className="px-3 md:px-6 py-4 bg-slate-50 min-h-[calc(100vh-64px)]">
                ...
                <div className="mb-3">
                    ...
                    <p className="text-xs md:text-sm text-slate-500">
                        Here's your latest academic analysis and personalized
                        study plan.
                    </p>
                </div>
                    ...
                    <div className="bg-white rounded-xl shadow-sm border border-slate-100 p-4 mt-2">
                        {planLoading ? (
                            ...
                            <p className="text-xs text-slate-500">
                                No marks found for your latest exam yet.
                            </p>
                        )}
                    </div>
                </div>
                        ...
                        <div className="bg-white rounded-xl shadow-sm border border-slate-100 p-4 h-full">
                            ...
                            {!latest ? (
                                ...
                                <div className="flex flex-col sm:flex-row items-center gap-3">
                                    <CircularGauge
                                        value={Number(
                                            latest?.predicted_risk_percentage ||
                                                0
                                        )}
